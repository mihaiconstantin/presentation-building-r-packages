...
You are requested to write a R package (`BootYourName`) that allows users to carry out a bootstrap analysis on the t statistic to test for equal means. The package will have to focus on the ordinary non-parametric bootstrapping procedure, i.e. the very basic one, and it will finally have to compare the bootstrap results with the theoretical ones (approximation of the statistic distribution to a Student t with specific degrees of freedom).
...
## **2. Package testing**   
Let's load our new package `BootYourName` and run the examples in the codes below
```{r, message=FALSE,warning=FALSE}
#library(BootYourName)
```
